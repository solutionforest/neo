...
//
// We spawn it with std::process::Command (NOT portable-pty): with no pre_exec
// hook, std uses posix_spawn on macOS, so there's no fork inside this
// multithreaded WebKit process (which modern macOS aborts). We also read the
// child's stdout as RAW BYTES — tauri-plugin-shell would line-buffer it, which
// breaks a terminal (no per-keystroke echo, prompts without a trailing newline
// like `$ ` or `password:` never appear).
...
pub fn pty_spawn(
    ...
    // stderr → frontend (raw); errors from the bridge surface in the terminal
    ...
    // Emit exit when stdout closes: spawn a waiter that reaps the child.
